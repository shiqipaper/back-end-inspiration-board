def test_create_board(client):
    response = client.post("/boards", json={"title": "Code Inspo", "owner": "Ada"})
    response_body = response.get_json()

    assert response.status_code == 201
    assert response_body == {
        "board": {"board_id": 1, "title": "Code Inspo", "owner": "Ada"}
    }
...
def test_create_board_with_extra_keys(client):
    response = client.post(
        "/boards",
        json={"extra": "some stuff", "title": "Code Inspo", "owner": "Grace Hopper"},
    )
    response_body = response.get_json()

    assert response.status_code == 201
    assert response_body == {
        "board": {"board_id": 1, "title": "Code Inspo", "owner": "Grace Hopper"}
    }
...
def test_get_single_board_one_saved_board(client, one_saved_board):
    ...
    assert len(response_body) == 1
    assert response_body == {
        "board": {"board_id": 1, "title": "Spread Kindness", "owner": "Grace"}
    }
...
def test_get_single_board_two_saved_boards(client, two_saved_boards):
    ...
    assert len(response_body) == 1
    assert response_body == {
        "board": {"board_id": 2, "title": "Book Quotes", "owner": "Alice Walker"}
    }


def test_create_card_with_board(client, one_saved_board):
    test_data = {"message": "Be kind"}
    ...
    assert response.status_code == 201
    assert response_body == {
        "card": {"card_id": 1, "message": "Be kind", "likes_count": 0, "board_id": 1}
    }
...
def test_create_card_missing_message(client, one_saved_board):
    ...
    assert response.status_code == 400
    assert response_body == {"message": "Invalid message"}
...
def test_create_card_message_exceeding_40_characters(client, one_saved_board):
    test_data = {"message": "Be the change you wish to see in this world"}

    response = client.post("/boards/1/cards", json=test_data)
    response_body = response.get_json()

    assert response.status_code == 400
    assert response_body == {"message": "Invalid message. Length is over 40 characters"}
...
def test_get_cards_by_board_expects_two_cards(client, board_with_two_cards):
    ...
    assert response_body == [
        ...
        {"card_id": 2, "message": "Hip, hip, array", "likes_count": 0, "board_id": 1},
    ]
